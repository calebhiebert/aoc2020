...
const TEST_DATA: &str = "light gold bags contain 2 light lime bags, 1 faded green bag, 3 clear olive bags, 2 dim bronze bags.
...
lazy_static! {
    static ref CONTAINED_RX: Regex = Regex::new(r"(\d+)\s([\w\s]+)\sbag[s]?[,\s]?.?").unwrap();
    static ref COLOR_RX: Regex = Regex::new(r"^([\w\s]+) bag[s]? contain").unwrap();
}
...
struct Bag {
    ...
    contained_by: Vec<(String, i32)>,
}

impl Bag {
    fn inner_bags(&self, bags: &HashMap<String, Bag>, count_root: bool) -> i64 {
        // println!("BAG {}", self.color);

        let mut sub_bags = self.contains
            .iter()
            .map(|inner_bag| (bags.get(&inner_bag.0), inner_bag.1))
            .filter_map(|b| {
                if b.0.is_some() {
                    Some((b.0.unwrap(), b.1))
                } else {
            ...
            .map(|b| {
                let ib = b.0.inner_bags(bags, true);

                // println!("  {} = {} x {} ({})", b.0.color, ib, b.1, ib * b.1 as i64);

                return ib * b.1 as i64;
            })
            .sum::<i64>();
        ...
        // println!("/BAG {} ({})", self.color, sub_bags);

        return sub_bags;
    }
}

fn parse_contained(ln: &str) -> Vec<(String, i32)> {
    CONTAINED_RX
        .captures_iter(ln)
...
fn parse_col(ln: &str) -> String {
    COLOR_RX
        .captures(ln)
        .unwrap()[1].to_string()
}

fn parse_bag_tree(input: &str) -> HashMap<String, Bag> {
    ...
    input
        ...
        .for_each(|ln| {
            ...
            let current_color = parse_col(ln);
            let contains = parse_contained(ln);

            contains.iter().for_each(|(color, count)| {
                if bags.contains_key(color) {
                    let col = bags.get_mut(color).unwrap();
                    col.contained_by.push((current_color.clone(), *count))
                } else {
                    let new_bag = Bag {
                        color: color.to_string(),
                        contains: vec![],
                        contained_by: vec![(current_color.clone(), *count)],
                    };

                    bags.insert(color.to_string(), new_bag);
                }

                if bags.contains_key(&current_color) {
                    let current_col = bags.get_mut(&current_color).unwrap();
                    current_col.contains.push((color.to_string(), *count));
                } else {
                    let new_bag = Bag {
                        color: current_color.clone(),
                        contains: vec![(color.to_string(), *count)],
                        contained_by: vec![],
                    };

                    bags.insert(current_color.clone(), new_bag);
                }
            });
        });
...
fn day_7_a(input: &str, start_color: &str) -> i32 {
    let bags = parse_bag_tree(input);

    let mut container_set: HashSet<String> = HashSet::new();
    let mut container_queue: VecDeque<String> = VecDeque::new();

    container_queue.push_back(start_color.to_string());

    while !container_queue.is_empty() {
        let bag = bags.get(&container_queue.pop_front().unwrap()).unwrap();

        bag.contained_by.iter().for_each(|c| {
            if !container_set.contains(&c.0) {
                container_set.insert(c.0.to_string());
                container_queue.push_back(c.0.to_string());
            }
        });
    }

    println!("SET {:?}", container_set);

    return container_set.len() as i32;
}

fn day_7_b(input: &str, start_color: &str) -> i64 {
    let bags = parse_bag_tree(input);
    ...
    bags.get(start_color).unwrap().inner_bags(&bags, false)
}

#[test]
fn test_day_7_a() {
    println!("{}", day_7_a(TEST_DATA, "shiny gold"));
}

#[test]
fn test_day_7_b() {
    println!("{}", day_7_b(TEST_DATA, "shiny gold"));
}