...
const TEST_DATA: &str = "edmzkxfoprcnhijtyvl
adxntojykfcvzermplh
    ...
    input
        ...
        .map(|grp| {
            ...
            grp
                ...
                // Add each char for group to set
                .for_each(|row| row
                    .chars()
                    .for_each(|c| {
                        set.insert(c);
...
fn day_6_b(input: &str) -> i64 {
    input
        // Split to groups
        .split("\n\n")
        .map(|grp| {
            let user_sets: Vec<HashSet<char>> = grp
                // Split to people
                .split("\n")
            ...
            let mut folded = user_sets[0].clone();

            for (i, set) in user_sets.iter().enumerate() {
                if i > 0 {
                    folded = folded.intersection(set).map(|c| *c).collect();
                }
            }

            return folded.len() as i64;
        })
...
#[test]
fn test_day_6_a() {
    println!("{}", day_6_a(TEST_DATA));
}

#[test]
fn test_day_6_b() {
    println!("{}", day_6_b(TEST_DATA));
}