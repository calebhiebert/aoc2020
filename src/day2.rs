...
const TEST_DATA: &str = "6-10 p: ctpppjmdpppppp
...
impl Entry {
    fn valid(&self) -> bool {
        let c: i32 = self.password.matches(&self.char).count() as i32;

        c >= self.min && c <= self.max
    }

    fn valid_b(&self) -> bool {
        let a = self.password.char_indices().nth(self.min as usize - 1).unwrap().1;
        let b = self.password.char_indices().nth(self.max as usize - 1).unwrap().1;
        let c = self.char.char_indices().nth(0).unwrap().1;

        (a == c && b != c) || (b == c && a != c)
    }
}

fn day_2_a(input: &str) -> Result<i32, &'static str> {
...
fn day_2_b(input: &str) -> Result<i32, &'static str> {
    let re = Regex::new(r"^(\d+)-(\d+)\s(\w):\s(.+)$").unwrap();
...
#[test]
fn test_day_2_a() {
    println!("{:?}", day_2_a(TEST_DATA))
}

#[test]
fn test_day_2_b() {
    println!("{:?}", day_2_b(TEST_DATA))
}