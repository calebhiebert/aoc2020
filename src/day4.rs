...
const TEST_DATA: &str = "iyr:2010 ecl:gry hgt:181cm
...
impl Height {
    fn parse(input: &str) -> Option<Height> {
        if input.ends_with("in") {
            return Some(Height::IN(input.strip_suffix("in").unwrap().parse().unwrap()));
        } else if input.ends_with("cm") {
            return Some(Height::CM(input.strip_suffix("cm").unwrap().parse().unwrap()));
        } else {
            return None;
        }
    }
...
impl Passport {
    fn extract(ln: &str) -> Passport {
        ...
        let ecl_rx = Regex::new(r"ecl:(\S+)").unwrap();
        let pid_rx = Regex::new(r"pid:(\d{9})").unwrap();
        let eyr_rx = Regex::new(r"eyr:(\S+)").unwrap();
        let hcl_rx = Regex::new(r"hcl:(#[0-9a-f]{6})").unwrap();
        let byr_rx = Regex::new(r"byr:(\S+)").unwrap();
        let iyr_rx = Regex::new(r"iyr:(\S+)").unwrap();
        let cid_rx = Regex::new(r"cid:(\S+)").unwrap();
        let hgt_rx = Regex::new(r"hgt:(\S+)").unwrap();

        if ecl_rx.is_match(ln) {
            passport.ecl = Some(ecl_rx.captures(ln).unwrap()[1].to_string());
        }

        if pid_rx.is_match(ln) {
            passport.pid = Some(pid_rx.captures(ln).unwrap()[1].to_string());
        }

        if eyr_rx.is_match(ln) {
            passport.eyr = Some(eyr_rx.captures(ln).unwrap()[1].to_string())
        }

        if hcl_rx.is_match(ln) {
            passport.hcl = Some(hcl_rx.captures(ln).unwrap()[1].to_string())
        }

        if byr_rx.is_match(ln) {
            passport.byr = Some(byr_rx.captures(ln).unwrap()[1].to_string())
        }

        if iyr_rx.is_match(ln) {
            passport.iyr = Some(iyr_rx.captures(ln).unwrap()[1].to_string())
        }

        if cid_rx.is_match(ln) {
            passport.cid = Some(cid_rx.captures(ln).unwrap()[1].to_string())
        }

        if hgt_rx.is_match(ln) {
            passport.hgt = Some(hgt_rx.captures(ln).unwrap()[1].to_string())
        }

        return passport;
    }

    fn multi_extract(input: &str) -> Vec<Passport> {
        input
            .split("\n\n")
            .map(Passport::extract)
            .collect()
    }

    fn valid(&self) -> bool {
        self.hgt.is_some() && self.iyr.is_some() &&
            self.pid.is_some() && self.byr.is_some() &&
            self.hcl.is_some() && self.eyr.is_some() &&
            self.ecl.is_some()
    }
    ...
    fn strict_valid(&self) -> bool {
        ...
        let hgt = Height::parse(&self.hgt.as_ref().unwrap());
        let valid: bool = match hgt {
            Some(Height::CM(cm)) => cm >= 150 && cm < 193,
            Some(Height::IN(inches)) => inches >= 59 && inches <= 76,
            None => false
        };
        if !valid {
            return false;
        ...
        if !vec!["amb", "blu", "brn", "gry", "grn", "hzl", "oth"].iter().any(|clr| *clr == self.ecl.as_ref().unwrap()) {
            return false;
        }
...
fn day_4_a(input: &str) -> i32 {
    Passport::multi_extract(input)
        .iter()
        .filter(|p| p.valid())
        .count() as i32
}

fn day_4_b(input: &str) -> i32 {
    Passport::multi_extract(input)
        .iter()
        .filter(|p| p.strict_valid())
        .count() as i32
}

#[test]
fn test_day_4_a() {
    println!("{}", day_4_a(TEST_DATA));
}

#[test]
fn test_day_4_b() {
    println!("{}", day_4_b(TEST_DATA));
}

#[test]
fn test_extraction() {
    println!("{:?}", Passport::multi_extract(TEST_DATA))
}